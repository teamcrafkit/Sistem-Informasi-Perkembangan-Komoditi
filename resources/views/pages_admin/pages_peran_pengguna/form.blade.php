                    ...
                    <div class="p-2">
                        <form class="form-horizontal parsley-form" action="{{ $route }}" method="post" enctype="multipart/form-data" autocomplete="off">
                            ...
                            <div class="form-group row">
                                <label class="col-form-label col-sm-2 text-sm-right" for="id">Nama Pengguna</label>
                                <div class="col-sm-4">
                                    <select type="text" name="id" id="id" class="form-control custom-select required" required>
                                        <option value="" selected>Pilih Pengguna</option>
                                        @foreach ($user as $item)
                                            @if (request()->segment(3) == "edit")
                                                <option {{ $data->id == $item->id ? 'selected' : '' }} value="{{ $item->id }}">{{ $item->nama }}</option>
                                            @else
                            ...
                            <div class="form-group row">
                                ...
                                <div class="col-sm-3">
                                    <select multiple="multiple" name="role[]" id="role" class="form-control custom-select " data-plugin="multiselect" >
                                        @foreach ($role as $item)
                                            @if (request()->segment(3) == "edit")
                                                <option {{ $data->roles()->find($item->id) ? 'selected' : '' }} value="{{ $item->id }}">{{ $item->name }}</option>
                                            @else
